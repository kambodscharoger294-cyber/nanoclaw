...
describe('SignalAdapter', () => {
  ...
  describe('inbound message handling', () => {
    ...
    it('forwards a captionless attachment sent to Note to Self instead of dropping it', async () => {
      const dataDir = join('/tmp', `signal-cli-test-data-${Date.now()}`);
      mkdirSync(join(dataDir, 'attachments'), { recursive: true });
      const attId = 'note-to-self-att1';
      writeFileSync(join(dataDir, 'attachments', attId), Buffer.from('fake pdf bytes'));
      ...
      pushEvent({
        ...
        syncMessage: {
          sentMessage: {
            timestamp: 1700000000000,
            destinationNumber: '+15551234567',
            attachments: [{ id: attId, contentType: 'application/pdf', filename: 'doc.pdf', size: 14 }],
          },
        },
      });
    ...
    it('still skips Note to Self sync messages with neither text nor attachments', async () => {
      const adapter = createAdapter();
      const cfg = createMockSetup();
      await adapter.setup(cfg);
